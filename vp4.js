// vp4.js - Orchestrator for the vacation planning workflow defined in vp4.json
...
async function main() {
  try {
    ...
    // Register tools used by the agents in vp4.json
    const webSearchToolInstance = agentFactory.registerTool(webSearchTool);
    const datetimeToolInstance = agentFactory.registerTool(dateTimeTool);
    const calculatorToolInstance = agentFactory.registerTool(calculatorTool);

    // Add validators and monitors (optional, but good practice)
    webSearchToolInstance.addValidator((params) => {
      if (!params.query || params.query.length < 3) {
        throw new Error('Search query must be at least 3 characters long');
      }
    });

    webSearchToolInstance.addMonitor((toolName, params, duration, success, result, error) => {
